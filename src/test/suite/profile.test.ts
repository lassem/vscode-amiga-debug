...
const binDir = path.resolve(__dirname, "../../../bin/" + process.platform + "/" + process.arch + "/opt/bin");
...
function test_profile_time(base: string, elf: string) {
	...
	const symbolTable = new SymbolTable(path.join(binDir, 'm68k-amiga-elf-objdump'), path.join(testDataDir, elf));
	const sourceMap = new SourceMap(path.join(binDir, 'm68k-amiga-elf-addr2line'), path.join(testDataDir, elf), symbolTable);
	...
	const json = profiler.profileTime(profileFile, Disassemble(path.join(binDir, 'm68k-amiga-elf-objdump'), path.join(testDataDir, elf)));
...
function test_profile_size(base: string, elf: string) {
	...
	const sourceMap = new SourceMap(path.join(binDir, 'm68k-amiga-elf-addr2line'), path.join(testDataDir, elf), symbolTable);
	const profiler = new Profiler(sourceMap, symbolTable);
	const json = profiler.profileSize(path.join(binDir, 'm68k-amiga-elf-objdump'), path.join(testDataDir, elf));
...
function test_unwind(elf: string) {
	const symbolTable = new SymbolTable(path.join(binDir, 'm68k-amiga-elf-objdump'), path.join(testDataDir, elf));
	const unwindTable = new UnwindTable(path.join(binDir, 'm68k-amiga-elf-objdump'), path.join(testDataDir, elf), symbolTable);
}
